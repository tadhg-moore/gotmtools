...
#'
#' Interpolates linearly ( or cubic spline) between model temperatures and extracts modelled temperatures at the same depths as observed temperatures.
#'
...
#' @param obs dataframe of observations loaded in using load.obs
...
setmodDepths <- function(mod.val, mod.dep, obs, method = 'linear', print = T){
  ...
  close(pb)
  df = data.frame(date = obs[,1], depths = dep, value = tmp)
  return(df)
}
